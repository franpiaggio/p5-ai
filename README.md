...
- **Multi-provider AI** — Choose between OpenAI (GPT-4o), Anthropic (Claude), or the free demo mode (Llama 3.3)
- **Monaco Editor** — Full-featured code editor with syntax highlighting, bracket matching, and error decorations
- **Live Preview** — Run sketches instantly with `Alt+Enter` and see results in a sandboxed iframe
- **Code Diff Review** — Review AI-suggested changes with a visual diff viewer before applying
...
DATABASE_PATH=./data/p5editor.sqlite
...
3. Configure your AI provider and API key in **Settings**
4. Review AI-suggested code changes in the diff viewer — accept with `Ctrl+Enter` or reject with `Esc`
